...
!
   ...
   do i = 1, n
      NumBlocks(i) = ptr2PG%Dsize(i)/box(i) !! Number of blocks of box(i) along Dim i
      ptr2CommGr%NumClusters = ptr2CommGr%NumClusters * NumBlocks(i) !! Product of all blocks.
      bid(i) = floor(real(ptr2PG%MyCoord(i)/box(i),kind=RealKind))   ! starting from 0
      color = color + ibase*bid(i) 
!!xDx!! The topology is sequential along boxes, with the heierarchy dir1 > dir2 > dir3
      key = key + jbase*mod(ptr2PG%MyCoord(i),box(i)) !! key provided using the same sequential topology as above. 
...
!!qDq!! What is ptr2PG%ptr2cg%Communicator. what does this split it into. 
!  -------------------------------------------------------------------
...
!!xDx!! MPI_IN_PLACE specifies that the inout is to be taken as the variable
!!xDx!! in the recv buffer, with the variable in the recv buffer also storing
!!xDx!! the result after the ALLREDUCE process.
!  -------------------------------------------------------------------
#endif
!
...
!
!  -------------------------------------------------------------------
   call allocateGroup(s) !! s= 'K-Mesh', 'Energy Mesh'
!  -------------------------------------------------------------------
!
   ptr2CommGr%NumClusters = 1
   color = 0; key = 0
...
!  -------------------------------------------------------------------
   call MPI_ALLREDUCE(MPI_IN_PLACE,ptr2CommGr%GroupMembers,n,         &
                      MPI_INTEGER,MPI_SUM,ptr2CommGr%Communicator,info)
!!xDx!! Each PE knows its rank and the rank of the other procs in the 
!!xDx!! group as stored in the array 'Group Member'.
!  -------------------------------------------------------------------
#endif
!
...
!  -------------------------------------------------------------------
   call MPI_ALLREDUCE(MPI_IN_PLACE,f,1,MPI_INTEGER,MPI_SUM,           &
                      ptr2CommGr%Communicator,info)!!xDx!!in_place makes snd and rcv buff the same
!  -------------------------------------------------------------------
#endif
!
...
!  ===================================================================
!!xDx!! Loops through all the groups created thus far, and if it finds
!!xDx!! the one with which the Index matches. If no match then it returns
!!xDx!! an error with the routine that called it (stored in sname).
   character (len=*), intent(in) :: sname
!
   integer (kind=IntKind), intent(in) :: i
   integer (kind=IntKind) :: n
