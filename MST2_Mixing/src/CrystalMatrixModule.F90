...
!  ===================================================================
   ICTXT = getGroupCommunicator(GroupID) !!xDx!! Set to 'Unit Cell'.
   call BLACS_GRIDINIT( ICTXT, 'R', 1, NumPEsInGroup ) !!a 1xNumPEsInGroup is created.
!!xDx!! BLACS_GRIDINIT( ICONTXT, Order, NPROW, NPCOL )
!!xDx!! ICTXT -> Not clear.
!!xDx!! Order ->  indicates the order (R -> RowMAjor C -> ColmnMajor)
!!xDx!! NPROWS -> No. of Process Rows the process grid should contian
!!xDx!! NPCOLS -> No. of Process COLs the process grid should contian
!  ===================================================================
   call BLACS_GRIDINFO(ICTXT, NPROW, NPCOL, MYROW, MYCOL)
!!xDx!! BLACS_GRIDINFO(ICONTXT, NPROW, NPCOL, MYROW, MYCOL)
!!xDx!! ICONTXT -> Integer HAndle denoting the Context. Context is  
!  -------------------------------------------------------------------
   if (NPROW /= 1 .or. NPCOL /= NumPEsInGroup .or. MYROW /= 0) then !!xDx!! standard things to indicate error.
!     ----------------------------------------------------------------
      call ErrorHandler('initCrystalMatrix',                                 &
              'Failed: NPROW /= 1 || NPCOL /= NumPEsInGroup || MYROW /= 0',  &
              NPROW, NPCOL, MYROW)
!     ----------------------------------------------------------------
   else if (MYCOL /= MyPEinGroup) then !!?D?!! Why is this a problem.
!     ----------------------------------------------------------------
...
!!xDx!! An MPI_Barrier for all procs within Communicator='GroupID'\equiv 'Unit Cell'.
!  -------------------------------------------------------------------
   call DESCINIT( DESC_A, KKRMatrixSizeCant, KKRMatrixSizeCant,       &
                  BandSizeCant, BandSizeCant, 0, 0, ICTXT,            &
                  KKRMatrixSizeCant, INFO )
!!xDx!! If the global matrix is of size A, the corresponding call would  initialize an array descriptor for A.
!!xDx!! DESC_A -> (o/p) Contains the descriptor vector.
!!xDx!! KKRMatrixSizeCant -> #Rows of global Matrix
!!xDx!! KKRMatrixSizeCant -> #Cols of Global matrix
!!xDx!! BandSizeCant -> #Rows in a block of A...
!!xDx!! No idea what this does.
!  -------------------------------------------------------------------
#endif
!
