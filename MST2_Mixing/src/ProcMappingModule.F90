...
!!xDx!! m is the output giving us the #of 3 factor groups.
      do i = 1, m
         if (NumAtoms_tmp >= factors(1,i) .and. NumKs >= factors(2,i) .and. &
            ...
            if (ra == 0) then   ! we only consider the case that atoms are evenly distributed
               ...
               nk = factors(2,i)
               ne = factors(3,i)
               nsum = NumAtoms_tmp/na + (NumKs-rk)/nk + rk + (NumEs-re)/ne + re !!xDx!!  as ra=0
!!nDn!! The way we are parallelizing the problem is by finding  3 factors na, nk, ne. These are
!!nDn!! the no. of procs in each dimension of the 3D Process Grid. We require na divide Total num of atoms.
!!nDn!! No such restricition on NumKs and NumEs. Thus if NumKs = rk (mod nk) then each process handles the
!!nDn!! calculation for these rk points (redundant calculation).
!              =======================================================
!              The following criteria for optimizing the distribution
!              needs to be further tested
!              =======================================================
               if (re+rk < re_sav+rk_sav) then !!xDx!! re_sav+rk_sav=NumEs+NumKs
...
!
   if (atom_distributed) then !!xDx!! For Cu, this one runs.
...
!
   NumKsPerBox = (NumKs-rk_sav)/nk_sav + rk_sav !!xDx!! As described in 556
   NumEsPerBox = (NumEs-re_sav)/ne_sav + re_sav !!xDx!! As described in 556
   na = na_sav; nk = nk_sav; ne = ne_sav
   ra = ra_sav; rk = rk_sav; re = re_sav
   np = NumAtoms/NumAtomsPerProc  ! = na !!xDx!! Num of procs along the Atom axis of Proc Grid.
!
...
!  -------------------------------------------------------------------
   call setupProcDim(np,proc_dim) !!?D?!! Not sure
!  -------------------------------------------------------------------
...
!!xDx!! Assuming that there are 8 Atoms in total, proc_dim \equiv pd
!!xDx!!  e-dir
!!xDx!!  A     ________________      ________________
!!xDx!!  |    |                |    |                |                        
!!xDx!!  |    |  O O ...  O O  |    |  O O ...  O O  |                        
!!xDx!!  |    |                |    |                |                        
!!xDx!!  |    |  O .        O  |    |  O .        O  |                        
...
!!xDx!!  |    |                |    |                | 
!!xDx!!  |    |  O O ...  O O  |    |  O O ...  O O  | 
!!xDx!!  |    |________________|    |________________| 
!!xDx!!  |                                             
!!xDx!!  |     ________________      ________________ 
!!xDx!!  |    |                |    |                |  
!!xDx!!  |  A |  O O ...  O O  |    |  O O ...  O O  |
!!xDx!!  |  | |                |    |                |          
!!xDx!!  |  | |  O .        O  |    |  O .        O  |
!!xDx!!  |  | |     .          |    |     .          |
!!xDx!!  |  p |  .   .      .  |    |  .   .      .  |
...
!!xDx!!  |  | |  O        . O  |    |  O        . O  |
!!xDx!!  |  | |                |    |                |
!!xDx!!  |  V |  O O ...  O O  |    |  O O ...  O O  |
!!xDx!!  |    |________________|    |________________|
!!xDx!!  |       <---pd(1)--->                         
!!xDx!!  -----------------------------------------------------------------> k-dir
!!xDx!!  The total no. of atoms is parallelized over the pd(1)xpd(2) box.
!!xDx!!  Each such box is repeated in the k-dir and e-dir nk and ne times
!!?D?!!  The third dimension pd(3) is kept the same
!
!  -------------------------------------------------------------------
   call createProcGrid(3,'3-D Proc Grid',proc_dim)
!!xDx!! creates a '3' dim Grid of processors titled '3-D Proc Grid' with
!!xDx!! integer array 'proc_dim' specifying no. of procs in each dim.
!  -------------------------------------------------------------------
   grid_id = getProcGridID('3-D Proc Grid')
!!xDx!! Returns the corresponding grid id. 
!  -------------------------------------------------------------------
...
!!xDx!! This sets each proc_dim(1)xproc_dim(2) box as a group.
!  -------------------------------------------------------------------
   if (NumKs > 0) then
...
!  ===================================================================
!!xDx!! num := Num Procs along Atom dim; np
   use PrimeFactorsModule, only : getSubFactors
!
   implicit none
!
